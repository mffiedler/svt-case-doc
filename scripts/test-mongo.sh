...
do
  echo ${i}
  oc exec $(oc get pod -n ttt | grep ycsb | awk '{print $1}') -- ./bin/ycsb load mongodb -s -P workloads/workloadt/workloadt -p mongodb.url=mongodb://redhat:redhat@${MONGO_IP}:27017/testdb
