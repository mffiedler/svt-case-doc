...
### glusterfs-storage
# oc process -f ./mongodb-persistent-template-ttt.yaml -p MEMORY_LIMIT=2048Mi -p MONGODB_USER=redhat -p MONGODB_PASSWORD=redhat -p MONGODB_DATABASE=testdb -p VOLUME_CAPACITY=10Gi -p STORAGE_CLASS_NAME=glusterfs-storage | oc create -f -
...
Need to clean the test data generated by `ycsb`. Our docker container does NOT support mongo client

```sh
# oc create -f https://raw.githubusercontent.com/hongkailiu/svt-case-doc/master/files/pod_mongo.yaml
# oc get pod
NAME              READY     STATUS    RESTARTS   AGE
m-cli             1/1       Running   0          14s
mongodb-2-4v2mb   1/1       Running   0          4h
ycsb-2-lsjt7      1/1       Running   0          46m

### This actually should be fixed in registry.access.redhat.com/rhscl/mongodb-32-rhel7:
# oc exec mongodb-2-4v2mb -- mongo --version
rpc error: code = 2 desc = oci runtime error: exec failed: container_linux.go:247: starting container process caused "exec: \"mongo\": executable file not found in $PATH"

command terminated with exit code 126

# oc exec m-cli -- mongo --version
...
Clean:
```sh
# oc exec m-cli -- mongo -u redhat -p redhat 172.24.183.75:27017/testdb --eval "db.usertable.remove({})"
```
